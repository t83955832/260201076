...
for x in sliced_eq_1:
    if 'x' in x:a+=(int(x[0:len(x)-1]))
    elif 'y' in x:b+=(int(x[0:len(x)-1]))
    else:c+=int(x)

if b<0:b_string=str(b)
else:b_string='+'+str(b)

for x in sliced_eq_2:
    if 'x' in x:d+=(int(x[0:len(x)-1]))
    elif 'y' in x: e+=(int(x[0:len(x)-1]))
    else:f+=int(x)
    
if e<0:e_string=str(e)
else: e_string='+'+str(e)
